...
let join_with_prespace_tests =
  Alcotest.
    [
      test_case "GIVEN empty list WHEN join_with_prespace THEN empty string"
        `Quick (fun _ ->
          (check string) "strings equal" "" (Dirsp_ps2ocamlcore.join_with_prespace []));
      test_case
        "GIVEN a,b,c list WHEN join_with_prespace THEN a,b,c with space before \
         each"
        `Quick (fun _ ->
          (check string) "strings equal" " a b c"
            (Dirsp_ps2ocamlcore.join_with_prespace [ "a"; "b"; "c" ]));
    ]

let overflow_underflow_tests =
  [
    Alcotest.test_case "GIVEN (a:=maxint)+1 WHEN eval_statement THEN overflow"
      `Quick (fun _ ->
        Alcotest.check_raises "Invalid_argument raised"
          (Invalid_argument "(a+1)=(4611686018427387903+1) will overflow")
          (fun () ->
            let s =
              Dirsp_ps2ocamlcore.eval_statement 0 emptyctx
            ...
            if
              ...
              <> "(*check s+t for overflow and underflow*) let s = a in let t \
                  ...
                  else raise (Invalid_argument (Format.sprintf \"(a+1)=(%d+%d) \
                  will overflow\" s t))) else if (s < 0) then (if (t >= \
                  Int.min_int - s) then () else raise (Invalid_argument \
            ...
            then Alcotest.fail ("UNEXPECTED: " ^ s)
              ...
              let t = 1 in
              if s > 0 then
                if t <= Int.max_int - s then ()
                else
                  raise
                    (Invalid_argument
                       (Format.sprintf "(a+1)=(%d+%d) will overflow" s t))
              else if s < 0 then
                if t >= Int.min_int - s then ()
                else
                  raise
                    (Invalid_argument
                       (Format.sprintf "(a+1)=(%d+%d) will underflow" s t))));
    Alcotest.test_case "GIVEN (a:=maxint-1)+1 WHEN eval_statement THEN ok"
      `Quick (fun _ ->
        let s =
          Dirsp_ps2ocamlcore.eval_statement 0 emptyctx
            (`Expression (add_expr variableA number1), emptyloc)
            ...
            if t >= Int.min_int - s then ()
              ...
              raise
                (Invalid_argument
                   (Format.sprintf "(a+1)=(%d+%d) will underflow" s t)));
    Alcotest.test_case
      "GIVEN (a:=minint)+(b:=maxint) WHEN eval_statement THEN ok" `Quick
      (fun _ ->
        let s =
          Dirsp_ps2ocamlcore.eval_statement 0 emptyctx
    ...
    Alcotest.test_case "GIVEN (a:=minint)+0 WHEN eval_statement THEN ok" `Quick
      (fun _ ->
        let s =
          Dirsp_ps2ocamlcore.eval_statement 0 emptyctx
      ...
      (fun _ ->
        ...
        if
          ...
          <> "(*check s-t for overflow and underflow*) let s = a in let t = 0 \
              ...
              else raise (Invalid_argument (Format.sprintf \"(a-0)=(%d - %d) \
        ...
        then Alcotest.fail ("UNEXPECTED: " ^ s)
          ...
          let t = 0 in
          if s > 0 then
            if t > 0 || -t <= Int.max_int - s then ()
            else
              raise
                (Invalid_argument
                   (Format.sprintf "(a-0)=(%d - %d) will overflow" s t))
          else if s < 0 then
            if t < 0 || (t <> Int.max_int && -t >= Int.min_int - s) then ()
            else
              raise
                (Invalid_argument
                   (Format.sprintf "(a-0)=(%d - %d) will underflow" s t)));
    Alcotest.test_case "GIVEN (a:=minint)-1 WHEN eval_statement THEN underflow"
      `Quick (fun _ ->
        Alcotest.check_raises "Invalid_argument raised"
          (Invalid_argument "(a-1)=(-4611686018427387904 - 1) will underflow")
          (fun () ->
            let s =
              Dirsp_ps2ocamlcore.eval_statement 0 emptyctx
            ...
            if
              s
              <> "(*check s-t for overflow and underflow*) let s = a in let t \
                  = 1 in (if (s > 0) then (if (t > 0 || -t <= Int.max_int - s) \
